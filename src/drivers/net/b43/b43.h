...
#define CONFIG_B43_DEBUG

#define CONFIG_B43_BCMA

// SSB Definitions
#define CONFIG_B43_SSB
#define CONFIG_B43_PCI_AUTOSELECT // no use but for a printout?

// fixme ipxe. The following lacks support
//#define CONFIG_B43_PCMCIA
//#define CONFIG_B43_SDIO


// NEW DEFS
#define CONFIG_B43_PHY_G // compiles but untested
/* N Phy tested on BCM4322 Apple Airport Extreme 2009 Macbook Pro
 * Only 2.4 GHZ rates tested so far. Will test others soon */
#define CONFIG_B43_PHY_N
#define CONFIG_B43_PHY_LP   // compiles but untested
#define CONFIG_B43_PHY_HT   // compiles but untested
#define CONFIG_B43_PHY_LCN  // compiles but untested


// No longer present in project (if ever?):
//#define CONFIG_B43_BUSES_SSB
// No longer present in project (if ever?):
//#define CONFIG_B43_PCICORE_AUTOSELECT
// No longer present in project (if ever?):
//#define CONFIG_B43_PIO
// No longer present in project
...
/* in ipxe, the hw encryption keys are not used at all. That is done in
 * software by ipxe. But if we leave this struct we can use the stock b43
 * key clear routines to clear out the keys from hardware memory */
struct b43_key {
	u8 algorithm;
};

/* SHM offsets to the QOS data structures for the 4 different queues. */
/* ipxe note: no QoS support in ipxe so only 1 queue */
#define B43_QOS_QUEUE_NUM	1
...
// Added for iPXE, stripped down Linux version
struct b43_linux_firmware {
	size_t size;
	const u8 *data;
};
...
struct b43_firmware_file {
	const char *filename;
	const struct b43_linux_firmware *data;
...
#define b43_set_status(wldev, stat)	do \
	{ DBGC(wldev, "status = %d\n", stat); \
	wldev->__init_status = (stat); barrier(); } while (0)
...
struct b43_wl {
	...
	struct list_head tx_queue[B43_QOS_QUEUE_NUM];
...
static inline struct net80211_channel *ndev_to_curchan(struct net80211_device *ndev)
{
	return ndev->channels + ndev->channel;
}

static inline struct b43_wl *ndev_to_b43_wl(struct net80211_device *ndev)
{
	return (struct b43_wl *)ndev->priv;
}

static inline struct net80211_device *b43_wl_to_ndev(struct b43_wl *wl)
{
	return wl->ndev;
}

static inline struct net80211_device *b43_wldev_to_ndev(struct b43_wldev *wldev)
{
	return b43_wl_to_ndev(wldev->wl);
}

/* unused but acts as an example how to get ssb driver data */
#if 0
/* the device handle is the one belonging to struct ssb_device */
static inline struct b43_wldev *dev_to_b43_wldev(struct device *dev)
{
	struct ssb_device *ssb_dev = dev_to_ssb_dev(dev);
	return (struct b43_wldev *)ssb_get_drvdata(ssb_dev);
}
#endif
 ...
 */
static inline int b43_current_band(struct b43_wl *wl)
{
	struct net80211_device *ndev = b43_wl_to_ndev(wl);
	return (ndev->channels + ndev->channel)->band;
}
...
enum b43_rate_control_flags {
	IEEE80211_RATE_SHORT_PREAMBLE	 = (1UL << 2),
};
////
///
//

// this struct defined in main.c, exported solely for xmit.c
extern const struct b43_rate __b43_ratetable[];


// necessary remnants from debugfs module
...
// stub
static inline bool b43_debug(struct b43_wldev *dev __unused,
                             enum b43_dyndbg feature)
{
	...
	switch(feature)
	{
		case B43_DBG_XMITPOWER:
		case B43_DBG_DMAOVERFLOW:
		case B43_DBG_DMAVERBOSE:
		case B43_DBG_PWORK_FAST:
		case B43_DBG_PWORK_STOP:
		case B43_DBG_LO:
		case B43_DBG_FIRMWARE:
		case B43_DBG_KEYS:
		case B43_DBG_VERBOSESTATS:
		default:
			enabled = false;
	}
