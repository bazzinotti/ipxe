...
#define B43_TXRING_SLOTS 64
#define B43_RXRING_SLOTS 64
/* the ipxe heap is 512 KB (check core/malloc.c). It's not big enough to
 * handle many slots that the stock driver uses. Inconclusive tests are
 * showing that even increasing the size of the heap to support the higher
 * slot numbers is not yielding a performance increase any better than
 * PIO. And with lower slot numbers, the speeds are worse than PIO. There
 * are other factors that may be causing these results such the rc80211
 * (rate control) code, the polling interval to the wireless driver (since
 * ipxe doesn't use interrupts for the wireless drivers), and other
 * unforeseen causes. */
