...
static void sprom_get_mac(unsigned char *mac, const u16 *in)
{
	int i;
	for (i = 0; i < 3; i++) {
		*mac++ = in[i] >> 8;
...
{
	pci_read_config_word(bus->host_pci, PCI_SUBSYSTEM_VENDOR_ID, &bi->vendor);
	pci_read_config_word(bus->host_pci, PCI_SUBSYSTEM_ID, &bi->type);
}
