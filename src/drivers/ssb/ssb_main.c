...
// fixme: This is not needed in iPXE, since there is no early boot delay to
...
// fixme ipxe this needs to get assigned to something
...
static void ssb_devices_unregister(struct ssb_bus *bus)
{
	...
	for (i = bus->nr_devices - 1; i >= 0; i--) {
		...
		if (sdev->dev)
    {
			ssb_release_dev(sdev->dev);
      //sdev->dev = NULL; // fixme, do I not need to do this?
    }
    if (sdev->drv)
      ssb_device_remove(sdev);
	}
}
...
/* Each core is tried for a driver */
static int ssb_devices_register(struct ssb_bus *bus)
{
    ...
    sprintf(dev->name,"ssb%x:%d:%s", bus->busnumber, dev_idx,
            ssb_core_name(sdev->id.coreid));
    ...
    /* Look for a driver */
    if ( ( err = ssb_find_driver ( sdev ) ) != 0 ) {
      DBGC ( sdev, SSB_FMT " (%s) has no driver\n",
            SSB_ARGS (sdev), dev->name);
      continue;
    }

    /* Probe the found driver */
    if ( ( err = ssb_probe ( sdev ) ) == 0 ) {
      /* ssbdev registered */
    } else {
      /* Not registered */
      ssb_err("Could not register %s\n", dev->name);

      // Set dev to NULL to not unregister dev on error unwinding.
      sdev->dev = NULL;
      kfree(devwrap);
      goto error;
    }
			    ...
			    unsigned long baseaddr)
{
	...
	/* nothing needed to be done in ipxe ;) */
	...
	// Queue it for attach.
	/* iPXE note: Since the SSB isn't implemented quite as strongly as an
	 * actual bus in iPXE (it's not attached to the root bus like the PCI
	 * and other buses are), we attach the bus immediately */
...
#ifdef CONFIG_SSB_PCIHOST
int ssb_bus_pcibus_register(struct ssb_bus *bus, struct pci_device *host_pci)
{
	int err;
