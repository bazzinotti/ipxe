...
struct ssb_sprom {
	u8 revision;
	u8 il0mac[6] __baligned(sizeof(u16));	/* MAC address for 802.11b/g */
	u8 et0mac[6] __baligned(sizeof(u16));	/* MAC address for Ethernet */
	u8 et1mac[6] __baligned(sizeof(u16));	/* MAC address for 802.11a */
...
struct ssb_device {
	...
	struct device *dev;
/* Because this codebase largely depended on struct device_driver; the
 * registered struct ssb_driver was not directly connected in any way.
 * Rather it was connected by including struct device_driver in the
 * ssb_driver object, and using container_of on the device_driver. Since
 * iPXE has no device_driver struct, and the bus system will be changing
 * to support iPXE and mirror PCI root probing, the ssb_device will mirror
 * PCI by putting ssb_driver into the ssb_device. The time that drv can be
 * specified into the ssb_device is during matching/probing when access to
 * both the device and driver is in the same scope. The network driver
 * calls ssb_driver_register, and the PCI driver prompted the device
 * registration. so iPXE will navigate the root bus by adding SSB to
 * whatever array the PCI and other buses are on */

/* Modeling PCI, there is a PCI driver table PCI_DRIVERS and struct
 * pci_driver are added on declaration using __pci_driver in the
 * declaration. SSB will model that, so that the network driver (b43) in
 * its ssb_driver decl needs __ssb_driver. Then at the device/driver
 * pairing stage, this drv field can be set appropriately. */
...
struct ssb_driver {
	const char *name;
	const struct ssb_device_id *ids;
...
#include <ipxe/ssb/ssb_driver_chipcommon.h>
#include <ipxe/ssb/ssb_driver_pci.h>
//#include <ipxe/ssb/ssb_driver_pcmcia.h>
//#include <ipxe/ssb/ssb_driver_sdio.h>
...
static inline void __ssb_dma_not_implemented(struct ssb_device *dev)
{
#ifdef CONFIG_SSB_DEBUG
